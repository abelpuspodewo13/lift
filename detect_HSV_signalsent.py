            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if names[int(c)] == "Up" or names[int(c)] == "Down":
                        #BOOLEAN TRUE
                        yajirushi = yajirushi + 1

                #Detect if arrive at 3rd Floor
                if third_floor > 0  and  yajirushi < 1:
                    #Send Signal to PLC
...
"""
Update Log
・20240529: name int c (detect ada angka 3 dan tidak ada yajirushi) nya ditaruh di luar while n> 1 (yg buat ngurangin double detection)
